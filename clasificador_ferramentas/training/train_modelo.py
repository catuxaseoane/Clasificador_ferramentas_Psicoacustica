...
from metodos.train_classifier import TrainClassifier
...
def main():
    ...
    X1 = np.load(os.path.join(load_path, 'datasetX1.npy'))
    X2 = np.load(os.path.join(load_path, 'datasetX2.npy'))
    X3 = np.load(os.path.join(load_path, 'datasetX3.npy'))
    y = np.load(os.path.join(load_path, 'labels.npy'))

    train_classifier = TrainClassifier(X,X1,X2,X3, y)
